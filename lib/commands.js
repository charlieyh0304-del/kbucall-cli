...
export async function login() {
  // 환경변수가 다 있으면 묻지 않는다. 터미널을 직접 쓸 수 없는 환경(스크립트, 파이프
  // 실행)에서 토큰을 처음 받으려면 이 경로가 필요하다.
  const envName = process.env.KBUCALL_NAME;
  const envPhone = process.env.KBUCALL_PHONE;
  const envPassword = process.env.KBUCALL_PASSWORD;
  const fromEnv = Boolean(envName && envPhone && envPassword);
  ...
  if (fromEnv) {
    ...
    if (!canAsk()) {
      throw new Error(
        '로그인하려면 직접 입력이 필요합니다. 터미널 창에서 실행하거나, '
        + 'KBUCALL_NAME, KBUCALL_PHONE, KBUCALL_PASSWORD 환경변수를 설정하세요.',
      );
    }
    line('복지콜 계정으로 로그인합니다. 복지콜 앱에 쓰는 이름, 전화번호, 비밀번호입니다.');
    memberName = await ask('이름: ');
    phone = await ask('전화번호: ');
    password = await askSecret('복지콜 비밀번호 (화면에 표시되지 않습니다): ');
  }
